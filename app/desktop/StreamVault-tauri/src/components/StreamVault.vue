      ...
      <div class="header-content">
        <div class="search-box">
          ...
          >
            ...
            <template #append>
              <el-button @click="handleSearch" style="width: 100px;">
                搜索
              </el-button>
            </template>
          </el-input>
  ...
  setup() {
    ...
    const loadMore = async () => {
      ...
      if (!settings.token || !settings.serverUrl) {
        ElMessage.warning('请先设置服务器地址和访问令牌')
        hasError.value = true
        return
      }
      ...
      loading.value = true
      try {
        const response = await invoke('fetch_videos', {
        ...
        if (response && response.resCode === '000001') {
          if (response.record?.content) {
            const newContent = response.record.content
            if (newContent.length > 0) {
              mediaList.value = [...mediaList.value, ...newContent]
              currentPage.value++
              total.value = response.record.totalElements || 0
...
.modern-search :deep(.el-input-group__append button) {
  border: none;
  height: 40px;
  color: white;
  padding: 0 20px;
...
.modern-search :deep(.el-input-group__append button:hover) {
